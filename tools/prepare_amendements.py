...
def process(OUTPUT_DIR, procedure):
    ...
    for i, step in enumerate(procedure['steps']):
        ...
        for loiid in get_text_id(texte_url), last_text_id:
            url_seances = 'https://{}.fr/seances/{}/json{}&{}'.format(urlapi, loiid, commission_or_hemicycle, CACHE_BUSTING)
            print('         * downloading seances - ', url_seances)
            for id_seance_obj in download(url_seances).json().get('seances', []):
                url_seance = 'https://{}.fr/seance/{}/{}/json?{}'.format(urlapi, id_seance_obj['seance'], loiid, CACHE_BUSTING)
