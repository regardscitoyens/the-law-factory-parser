...
re_mat_end = re.compile(r"(<i>Délibéré|Fait à .*, le|\s*©|\s*N.?B.?\s*:|(</?i>)*<a>[1*]</a>\s*(</?i>)*\(\)(</?i>)*|<a>\*</a>\s*<i>1<i>\s*Not[ea])", re.I)
...
re_mat_texte = re.compile(r'\(texte d', re.I)
...
re_clean_footer_notes = re.compile(r" *\(?\d?\([\d\*]\)\)?\.$")
