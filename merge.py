...
def merge_senat_with_an(senat, an):
    ...
    for i, step in enumerate(senat['steps']):
        ...
        if step.get('stage') == 'promulgation' and (not step['source_url'] or 'pdf_jo' in step['source_url']):
            an_step_promulgation = [s for s in an['steps'] if s.get('stage') == 'promulgation']
            if an_step_promulgation and an_step_promulgation['source_url']:
                step['source_url'] = an_step_promulgation['source_url']
...
pp(merge_senat_with_an(json.load(open('data/parsed/senat/pjl13-215')), json.load(open('data/parsed/an/14-collectif-budgetaire-2013'))))

# strange CMP.hemicycle overwrite: assemblee CMP url stay void
pp(merge_senat_with_an(json.load(open('data/parsed/senat/pjl11-187')), json.load(open('data/parsed/an/13-limite-age-magistrats-ordre-judiciaire'))))

# complete AN urls
# pp(merge_senat_with_an(json.load(open('data/parsed/senat/pjl11-497')), json.load(open('data/parsed/an/14-accord-serbie-cooperation-policiere'))))

# complete AN steps
# pp(merge_senat_with_an(json.load(open('data/parsed/senat/pjl08-248')), json.load(open('data/parsed/an/13-kenya-imposition-fraude'))))
# pp(merge_senat_with_an(json.load(open('data/parsed/senat/pjl10-515')), json.load(open('data/parsed/an/13-accord-fiscal-dominique'))))
...
if __name__ == '__main__':
    ...
    for dos in all_senat:
        ...
        def merge_an_dos(dos, an):
            ...
            matched.append(merge_senat_with_an(dos, an))
